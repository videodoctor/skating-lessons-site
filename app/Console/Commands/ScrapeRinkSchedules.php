...
class ScrapeRinkSchedules extends Command
{
    ...
    private function processBrentwoodPdf(Rink $rink, string $storagePath, string $monthName, int $year): int
    {
        ...
        file_put_contents($tempPdf, Storage::get($storagePath));
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            if (preg_match('/^(\d{1,2})(\s|$)/', $line, $dm)) {
                ...
                if ($d >= 1 && $d <= 31) {
                    $currentDate = $d;
                    // If ONLY a date number, skip to next line
    ...
    private function scrapeWebsterGroves(Rink $rink)
    {
        ...
        foreach ($links as $link) {
            $href = $link->getAttribute('href');
            $txt  = trim($link->textContent);
            if (!preg_match('/(January|February|March|April|May|June|July|August|September|October|November|December)\s+(Skate\s+)?Calendar/i', $txt, $m)) continue;
            if (!preg_match('/\/DocumentCenter\/View\/(\d+)/', $href, $dm)) continue;
            $monthName    = $m[1];
            $matchedMonth = (int)date('n', strtotime($monthName . ' 1'));
            $year         = ($matchedMonth < (int)date('n')) ? (int)date('Y') + 1 : (int)date('Y');
            $schedules[$dm[1]] = ['month' => $monthName, 'year' => $year, 'url' => 'https://www.webstergrovesmo.gov' . $href, 'doc_id' => $dm[1]];
        }

        if (empty($schedules)) { $this->warn("  No PDFs found for Webster Groves"); return; }
        ...
        foreach ($schedules as $docId => $s) {
            $month             = (int)date('n', strtotime($s['month'] . ' 1'));
            $isCurrentOrFuture = Carbon::create($s['year'], $month, 1)->gte(now()->startOfMonth());
            $cacheKey          = "processed_webster_doc_{$docId}";
            ...
            try {
                ...
                $storagePath = $this->storeRawSourceVersioned($rink, $month, $s['year'], $docId, $pdfContent, 'pdf');
                $this->log("    Stored: {$storagePath}");
                $added = $this->processWebsterGrovesPdf($rink, $storagePath, $s['month'], $s['year']);
                $this->recordScrapeRun($rink, $month, $s['year'], $storagePath, 'pdf', $s['url'], $added, $added, 0);
                cache([$cacheKey => true], now()->addDays(60));
            } catch (\Exception $e) {
                $this->error("    Failed: {$e->getMessage()}");
